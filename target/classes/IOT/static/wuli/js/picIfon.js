...
function listIPCPointIMG(id,num,houTime) {
	...
	$.ajax({
		...
		success: function(data) {
			removeload();
			if(data.state == 0) {
				var str = '<div class="swiper-container gallery-top swiper-container-horizontal"><div class="swiper-wrapper2">';
				var strs='<div class="swiper-container gallery-thumbs swiper-container-horizontal"><div class="swiper-wrapper">';
				str += '<div class="swiper-slide swiper-slide-active"><img src="' + http + 'getReportImage?file_name=' + data.object[0].fileName + '"  class="swiper-lazy" alt=""/></div>'
				$(data.object).each(function(i, el) {
					var strImg = "";
					...
					strs+='<div class="swiper-slide swiper-slide-active"><img datasrc="' + strImg + '" src="' + mohuimg + '" alt=""/></div>'
				});
				str += '</div></div>';
				...
				// 点击小图改变大图
				document.querySelector('.pics_cont_picbotOut>.pics_cont_picbot>.swiper-container>.swiper-wrapper').onclick = function (e) {
					if(e.toElement.localName === 'img'){
// 						console.log(e.target);
// 						console.log(e.target.getAttribute('datasrc'));
						document.querySelector('.menuPage>.pics_content>.swiper-container>.swiper-wrapper2 img').src = e.target.getAttribute('datasrc');
