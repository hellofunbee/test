...
//	}
	...
	public List<Map<String, Object>> listMessage(MessageEntity me) {
		String sql ="select m.*,sa.a_name province, s.a_name city,sar.a_name district from message m left join s_area sa on sa.ar_id=m.m_province LEFT JOIN s_area s on s.ar_id =m.m_city LEFT JOIN s_area sar on sar.ar_id =m.m_district where 1=1 ";
		ArrayList<Object> list = new ArrayList<>();
		if(me.getM_id()>0){
			sql +=" and m_id =?";
			list.add(me.getM_id());
...
//		}
	...
	public List<Map<String, Object>> getInstanceMessage() {
		String sql ="select * from message where m_type = 2 order by m_time desc limit 0,3";
	...
	public List<Map<String, Object>> listMessage4(MessageEntity me, String string) {
		String sql ="select m.m_title,m.m_time,m.m_province,m.m_city,m.m_district,sa.a_name province, s.a_name city,sar.a_name district from message m left join s_area sa on sa.ar_id=m.m_province LEFT JOIN s_area s on s.ar_id =m.m_city LEFT JOIN s_area sar on sar.ar_id =m.m_district where 1=1 ";
		ArrayList<Object> list = new ArrayList<>();
		if(me.getM_id()>0){
			sql +=" and m_id =?";
			list.add(me.getM_id());
		...
		sql +=" group by "+string+" order by m_time desc  ";
		if(me.getM_type()==1 && me.getStart()>0){
			sql +=" limit "+(me.getStart()-1)*me.getPageSize()+","+me.getPageSize();
		}
		return jdbcTemplate.queryForList(sql,list.toArray());
	...
	public List<Map<String, Object>> listMessage5(MessageEntity me, String string) {
		String sql ="select m.m_id,m_content from message m left join s_area sa on sa.ar_id=m.m_province LEFT JOIN s_area s on s.ar_id =m.m_city LEFT JOIN s_area sar on sar.ar_id =m.m_district where 1=1 ";
		ArrayList<Object> list = new ArrayList<>();
		if(me.getM_id()>0){
			sql +=" and m_id =?";
			list.add(me.getM_id());
