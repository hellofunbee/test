...
public class ClassController {
	...
	// public IOTResult queryClass(@RequestBody ClassRequest c){
	...
	// }
	// if(c.getC_lev()>1&&c.getC_rid()<1){
	// return new IOTResult(false, "请输入父类目id", null, 0);
	// }
	// if(c.getC_lev()==1){
	// c.setC_rid(0);
	// }
	// List<ClassEntity> list = service.queryAllClass(c);
	// if(list.size()<1){
	...
	// @RequestMapping(value="/listClass1",method=RequestMethod.POST)
	// public IOTResult listClass1(@RequestBody ClassRequest c){
	// if(c.getCksid()==null||c.getCksid().trim().length()<1||c.getCkuid()==null||c.getCkuid().trim().length()<1){
	// return new IOTResult(false, "信息不规范", null, 1);
	// }
	// String check = tool.getCheck(ToolUtil.IOT+c.getCkuid());
	// if (check ==null||!check.equals(c.getCksid())) {
