...
public class IPCController {
	...
	public IOTResult addIPC(@RequestBody IPCRequest ipcRequest) {
		...
		// ipcRequest.getCksid().trim().length()<1||ipcRequest.getCkuid()==null||ipcRequest.getCkuid().trim().length()<1){
		// return new IOTResult(false,"信息不规范",null,1);
		// }
		// // 注册登陆按照什么来????
		// String check = toolUtil.getCheck(ToolUtil.IOT+ipcRequest.getCkuid());
		// if(check ==null ||!ipcRequest.getCksid().equals(check)){
		// return new IOTResult(false,"登陆失效",null,2);
		// }
		// long uid = toolUtil.getbase_uidSid(ipcRequest.getCkuid(),
		// ipcRequest.getCksid());
		// ipcRequest.getPointEntity().setUid(uid);
		// PointEntity pointEntity2 = new PointEntity();
		//// pointEntity2.setUid(uid);
		//// pointEntity2.setRole(String.valueOf(uid));
		// pointEntity2.setTp_id(ipcRequest.getPointEntity().getTp_pid());
		// PointEntity point = pointService.getPoint(pointEntity2);
		// if(point==null){
	...
	public IOTResult autoSyn(@RequestBody IPCRequest ipcRequest) {
		if (ipcRequest.getCksid() == null || ipcRequest.getCksid().trim().length() < 1 || ipcRequest.getCkuid() == null
				|| ipcRequest.getCkuid().trim().length() < 1) {
			return new IOTResult(false, "信息不规范", null, 1);
		}
		// 注册登陆按照什么来????
		String check = toolUtil.getCheck(ToolUtil.IOT + ipcRequest.getCkuid());
		...
		long uid = toolUtil.getbase_uidSid(ipcRequest.getCkuid(), ipcRequest.getCksid());
		byte b = 1;
	...
	public IOTResult deleteIPC(@RequestBody IPCRequest ipcRequest) {
		...
		// pointEntity.setTp_id(ipcRequest.getId());
		...
		Map<String, Object> ipc = ipcService.getIPCById(ipcRequest);
		if (ipc == null || ipc.get("deviceId") == null || ipc.get("deviceId").toString().trim().length() < 1) {
			return new IOTResult(false, "IPC不存在", null, 4);
		}
		int addIPC = ipcService.deleteIPC(ipcRequest);
		String config = "s_nod:" + ipc.get("s_nod") + ";s_ip:" + ipc.get("s_ip") + ";s_port:" + ipc.get("s_port")
				+ ";s_username:" + ipc.get("s_username") + ";s_password:" + ipc.get("s_password") + ";";
		if (addIPC > 0) {
	...
	public IOTResult updateIPC(@RequestBody IPCRequest ipcRequest) {
		...
		if (addIPC < 1) {
			// boolean setIpc12 = Client.setIpc1("edit", config,
			// ipcRequest.getDeviceId(), ipcRequest.getPointEntity().getIp(),
			// ipcRequest.getPointEntity().getPort());
			// if(setIpc12){
	...
	public IOTResult updateIPCPrarm(@RequestBody IPCRequest ipcRequest) {
		...
		// int updatePoint = pointService.updatePoint(pointEntity);
	...
	public IOTResult getIPCSetting(@RequestBody IPCRequest ipcRequest) {
		...
		// 条件代表还没有
		System.out.println(proxy.get("username").toString());
		System.out.println(proxy.get("password").toString());
		System.out.println(ipcRequest.getPointEntity().getIp());
		System.out.println(proxy.get("s_proxy").toString());
		System.out.println(proxy.get("deviceId").toString());
		System.out.println(proxy.get("deviceId").toString());
		System.out.println(ipcRequest.getPointEntity().getIp());
		System.out.println(proxy.get("s_proxy").toString());
	...
	public IOTResult getAppIPCPoint(@RequestBody IPCPointRequset ipcPointRequset) {
		if (ipcPointRequset.getCksid() == null || ipcPointRequset.getCksid().trim().length() < 1
				|| ipcPointRequset.getCkuid() == null || ipcPointRequset.getCkuid().trim().length() < 1) {
			return new IOTResult(false, "信息不规范", null, 1);
		}
		// 注册登陆按照什么来????

		String check = toolUtil.getCheck(ToolUtil.IOT + ipcPointRequset.getCkuid());
		if (check == null || !ipcPointRequset.getCksid().equals(check)) {
			return new IOTResult(false, "登陆失效", null, 2);
		}
		long uid = toolUtil.getbase_uidSid(ipcPointRequset.getCkuid(), ipcPointRequset.getCksid());
	...
	public IOTResult addIPCPoint(@RequestBody IPCPointRequset ipcPointRequset) {
		/*
		 * if(ipcPointRequset.getCksid()==null ||
		 * ipcPointRequset.getCksid().trim().length()<1||ipcPointRequset.
		 * getCkuid()==null||ipcPointRequset.getCkuid().trim().length()<1){
		 * return new IOTResult(false,"信息不规范",null,1); } // 注册登陆按照什么来???? String
		 * check = toolUtil.getCheck(ToolUtil.IOT+ipcPointRequset.getCkuid());
		 * if(check ==null ||!ipcPointRequset.getCksid().equals(check)){ return
		 * new IOTResult(false,"登陆失效",null,2); }
		 * 
		 * long uid = toolUtil.getbase_uidSid(ipcPointRequset.getCkuid(),
		 * ipcPointRequset.getCksid()); PointEntity pointEntity =
		 * ipcPointRequset.getPointEntity(); pointEntity.setUid(uid); //
		 * pointEntity.setRole(String.valueOf(uid));
		 * pointEntity.setTp_id(ipcPointRequset.getId()); PointEntity point =
		 * pointService.getPoint(pointEntity); if(point==null){ return new
		 * IOTResult(false,"节点不存在",null,3); } int ckAdmin =
		 * userService.ckAdmin(uid); if(ckAdmin ==0 ){ return new
		 * IOTResult(false,"权限不足",null,111); }
		 */
	...
	public IOTResult updateIPCPoint(@RequestBody IPCPointRequset ipcPointRequset) {
		...
		long uid = toolUtil.getbase_uidSid(ipcPointRequset.getCkuid(), ipcPointRequset.getCksid());
		// PointEntity pointEntity = ipcPointRequset.getPointEntity();
		// pointEntity.setUid(uid);
		// pointEntity.setRole(String.valueOf(uid));
		// pointEntity.setTp_id(ipcPointRequset.getId());
		// PointEntity point = pointService.getPoint(pointEntity);
		// if(point==null){
		// return new IOTResult(false,"节点不存在",null,3);
		// }
	...
	public IOTResult updateAppIPCPoint(@RequestBody IPCPointRequset ipcPointRequset) {
		// if(ipcPointRequset.getCksid()==null ||
		// ipcPointRequset.getCksid().trim().length()<1||ipcPointRequset.getCkuid()==null||ipcPointRequset.getCkuid().trim().length()<1){
		// return new IOTResult(false,"信息不规范",null,1);
		// }
		// // 注册登陆按照什么来????
		// String check =
		// toolUtil.getCheck(ToolUtil.IOT+ipcPointRequset.getCkuid());
		// if(check ==null ||!ipcPointRequset.getCksid().equals(check)){
		// return new IOTResult(false,"登陆失效",null,2);
		// }
		// long uid = toolUtil.getbase_uidSid(ipcPointRequset.getCkuid(),
		// ipcPointRequset.getCksid());
		// PointEntity pointEntity = ipcPointRequset.getPointEntity();
		// pointEntity.setUid(uid);
		// pointEntity.setRole(String.valueOf(uid));
		// pointEntity.setTp_id(ipcPointRequset.getId());
	...
	public IOTResult deleteIPCPointBatch(@RequestBody IPCPointsRequest points) {
		if (points.getCksid() == null || points.getCksid().trim().length() < 1 || points.getCkuid() == null
				|| points.getCkuid().trim().length() < 1) {
			return new IOTResult(false, "信息不points", null, 1);
		}
		// a
		// 注册登陆按照什么来????
		String check = toolUtil.getCheck(ToolUtil.IOT + points.getCkuid());
		if (check == null || !points.getCksid().equals(check)) {
			return new IOTResult(false, "登陆失效", null, 2);
		}
		long uid = toolUtil.getbase_uidSid(points.getCkuid(), points.getCksid());
		int ckAdmin = userService.ckAdmin(uid);
		if (ckAdmin == 0) {
			return new IOTResult(false, "权限不足", null, 111);
		}
